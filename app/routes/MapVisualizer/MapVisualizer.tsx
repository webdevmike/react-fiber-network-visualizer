  ...
  return (
    <>
                ...
                {FIBER_ROUTE_TYPES.map((type) => (
                  <li key={type}>
                    <FilterCheckbox
                      ...
                      onChange={(e) => toggleRouteType(type, e.target.checked)}
                      label={type}
                    />
                  </li>
                ))}
                ...
                {FIBER_NODE_TYPES.map((type) => (
                  <li key={type}>
                    <FilterCheckbox
                      ...
                      onChange={(e) => toggleNodeType(type, e.target.checked)}
                ...
                {STATUS.map((type) => (
                  <li key={type}>
                    <FilterCheckbox
                      ...
                      onChange={(e) => toggleStatus(type, e.target.checked)}
                      label={type}
                    />
                  </li>
                ))}
      ...
      <div className={styles.mobileMessage}>
        <p>
          Mobile view coming soon. For now, please use a device with a viewport
          wider than 800px.
        </p>
      </div>
    </>
